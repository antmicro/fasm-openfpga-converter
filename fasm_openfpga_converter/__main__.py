...
def main():
    ...
    parser.add_argument('--fasm2openfpga', action='store_const', const=True,
                        help='When set fasm is treated as input, and bustream \
                              is produced. If not set, bitstream is read and \
                              fasm is produced')
    parser.add_argument('--fasm', type=str, help='Path to SDF file',
                        required=True)
    parser.add_argument('--bitstream', type=str, help='Path to JSON file',
                        required=True)
    parser.add_argument('--device-independent', action='store_const',
                        const=True,
    ...
    if args.fasm2openfpga:
        with open(args.fasm) as fasm_file:
            ...
            xml.write(open(args.bitstream, 'wb'), encoding='utf-8',
                      pretty_print=True)
